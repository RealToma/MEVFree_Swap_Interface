...
import { MixedRoute, partitionMixedRouteByProtocol, Protocol, Trade } from 'mevswap/router-sdk'
...
const MAX_AUTO_SLIPPAGE_TOLERANCE = new Percent(100, 100) // 100%
 ...
 * Returns slippage tolerance based on values from current trade, gas estimates from api, and active network.
 */
export function useLegacyAutoSlippageTolerance(trade: V2Trade<Currency, Currency, TradeType> | null): Percent {
  const { chainId } = useWeb3React()
  const onL2 = chainId && L2_CHAIN_IDS.includes(chainId)
