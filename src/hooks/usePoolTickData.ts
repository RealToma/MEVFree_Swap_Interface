...
import { FeeAmount, nearestUsableTick, Pool, TICK_SPACINGS, tickToPrice } from 'mevswap/v3-sdk'
...
function useTicksFromTickLens(
  ...
  const poolAddress =
    currencyA && currencyB && feeAmount && poolState === PoolState.EXISTS
      ? Pool.getAddress(currencyA?.wrapped, currencyB?.wrapped, feeAmount, chainId ? PAIR.FACTORY_V3 : undefined)
      : undefined
