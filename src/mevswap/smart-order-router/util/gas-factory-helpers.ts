...
export async function getHighestLiquidityV3USDPool(chainId: ChainId, poolProvider: IV3PoolProvider): Promise<Pool> {
  ...
  const maxPool = _.maxBy(pools, (pool) => pool.liquidity) as unknown as Pool
...
export async function getHighestLiquidityUSDV2Pool(chainId: ChainId, poolProvider: IV2PoolProvider): Promise<Pair> {
  const usdTokens = usdGasTokensByChain[chainId]
