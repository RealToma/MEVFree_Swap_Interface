...
import { encodeMixedRouteToPath, MixedRouteSDK, Protocol } from 'mevswap/router-sdk'
import { encodeRouteToPath } from 'mevswap/v3-sdk'
...
export class OnChainQuoteProvider implements IOnChainQuoteProvider {
  ...
  private getQuoterAddress(useMixedRouteQuoter: boolean): string {
    ...
    if (!quoterAddress) {
      throw new Error(`No address for Uniswap QuoterV2 Contract on chain id: ${this.chainId}`)
    }
    ...
    const {
      ...
      async (_bail, attemptNumber) => {
        ...
        if (failedQuoteStates.length > 0) {
          ...
          if (
            (this.chainId === ChainId.ARBITRUM_ONE || this.chainId === ChainId.ARBITRUM_RINKEBY) &&
        ...
        return {
          ...
          blockNumber: BigNumber.from(callResults[0]?.blockNumber ? callResults[0].blockNumber : 0),
