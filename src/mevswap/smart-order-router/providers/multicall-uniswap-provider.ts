...
import { UniswapInterfaceMulticall } from 'mevswap/redux-multicall/abi/types'
...
export class UniswapMulticallProvider extends IMulticallProvider<UniswapMulticallConfig> {
  ...
  constructor(protected chainId: ChainId, protected provider: BaseProvider, protected gasLimitPerCall = 1_000_000) {
    ...
    if (!PAIR.MULTICALL_ADDRESS) {
      throw new Error(`No address for Multicall Contract on chain id: ${chainId}`)
    }

    this.multicallContract = UniswapInterfaceMulticall__factory.connect(PAIR.MULTICALL_ADDRESS, this.provider)
  }
