...
import { Protocol, SwapRouter, Trade } from 'mevswap/router-sdk'
import { Currency, Fraction, Token, TradeType } from 'mevswap/sdk-core'
import { PAIR, Pair, SupoprtedRouterType } from 'mevswap/v2-sdk'
import { MethodParameters, Pool, Position, SqrtPriceMath, TickMath } from 'mevswap/v3-sdk'
...
export class AlphaRouter implements IRouter<AlphaRouterConfig>, ISwapToRatio<AlphaRouterConfig, SwapAndAddConfig> {
  ...
  constructor({
    ...
    this.gasPriceProvider =
      ...
      new CachingGasStationProvider(
        ...
        new NodeJSCache<GasPrice>(new NodeCache({ stdTTL: 15, useClones: false }))
      )
    ...
    if (chainId === ChainId.OPTIMISM || chainId === ChainId.OPTIMISTIC_KOVAN || chainId === ChainId.BASE) {
  ...
  public async route(
    ...
    // if (
    //   (protocolsSet.size === 0 || (protocolsSet.has(Protocol.V2) && protocolsSet.has(Protocol.V3))) &&
    //   V2_SUPPORTED.includes(this.chainId)
    // ) {
    //   log.info({ protocols, tradeType }, 'Routing across all protocols')
    //   //console.log('RoutingAcross all protocols')
    //   quotePromises.push(
    //     this.getV3Quotes(tokenIn, tokenOut, amounts, percents, quoteToken, v3gasModel, tradeType, routingConfig)
    //   )
    //   quotePromises.push(
    //     this.getV2Quotes(tokenIn, tokenOut, amounts, percents, quoteToken, gasPriceWei, tradeType, routingConfig)
    //   )
    ...
    if (PAIR.ROUTER_TYPE === SupoprtedRouterType.V3 || PAIR.ROUTER_TYPE === SupoprtedRouterType.V2V3) {
      log.info({ protocols, swapType: tradeType }, 'Routing across V3')
      //console.log('RoutingAcross V3 protocol')
      quotePromises.push(
        this.getV3Quotes(tokenIn, tokenOut, amounts, percents, quoteToken, v3gasModel, tradeType, routingConfig)
      )
    }
    ...
    if (PAIR.ROUTER_TYPE === SupoprtedRouterType.V2 || PAIR.ROUTER_TYPE === SupoprtedRouterType.V2V3) {
      ...
      //console.log('RoutingAcross V2 protocol')
      quotePromises.push(
        this.getV2Quotes(tokenIn, tokenOut, amounts, percents, quoteToken, gasPriceWei, tradeType, routingConfig)
      )
    }
    ...
    //   PAIR.ROUTER_TYPE === SupoprtedRouterType.V2V3 &&
    //   (this.chainId === ChainId.MAINNET || this.chainId === ChainId.GÖRLI) &&
    //   tradeType === TradeType.EXACT_INPUT
    // ) {
    //   log.info({ protocols, swapType: tradeType }, 'Routing across MixedRoutes')
  ...
  private async getV3Quotes(
    ...
    const routesWithValidQuotes: any[] = []
  ...
  private async getV2Quotes(
    ...
    //console.log('Starting to get V2 quotes')
    // Fetch all the pools that we will consider routing via. There are thousands
    // of pools, so we filter them to a set of candidate pools that we expect will
    // result in good prices.
    ...
    //console.log('computeAllV2Routes poolAccessor: ', poolAccessor)
    //console.log('computeAllV2Routes candidatePools: ', candidatePools)
    //console.log('computeAllV2Routes pools: ', pools)
    //console.log('computeAllV2Routes routes: ', routes)
    ...
    console.log(`Getting quotes for V2 for ${routes.length} routes with ${amounts.length} amounts per route.`)
