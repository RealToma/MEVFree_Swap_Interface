...
import { partitionMixedRouteByProtocol } from 'mevswap/router-sdk'
import { Pair } from 'mevswap/v2-sdk'
import { Pool } from 'mevswap/v3-sdk'
...
export class MixedRouteHeuristicGasModelFactory extends IOnChainGasModelFactory {
  ...
  public async buildGasModel({
    ...
    const usdPool: Pool = await getHighestLiquidityV3USDPool(chainId, V3poolProvider)
    ...
    const nativeV3Pool: Pool | null = await getHighestLiquidityV3NativePool(chainId, token, V3poolProvider)
  ...
  private estimateGas(routeWithValidQuote: MixedRouteWithValidQuote, gasPriceWei: BigNumber, chainId: ChainId) {
