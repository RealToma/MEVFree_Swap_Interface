...
export class V2HeuristicGasModelFactory extends IV2GasModelFactory {
  ...
  public async buildGasModel({
    ...
    if (token.equals(WRAPPED_NATIVE_CURRENCY[chainId])) {
      const usdPool: Pair = await this.getHighestLiquidityUSDPool(chainId, poolProvider)
    ...
    const ethPool: Pair | null = await this.getEthPool(chainId, token, poolProvider)
    if (!ethPool) {
      log.info(
  ...
  private estimateGas(routeWithValidQuote: V2RouteWithValidQuote, gasPriceWei: BigNumber, chainId: ChainId) {
  ...
  private async getEthPool(chainId: ChainId, token: Token, poolProvider: IV2PoolProvider): Promise<Pair | null> {
    const weth = WRAPPED_NATIVE_CURRENCY[chainId]
  ...
  private async getHighestLiquidityUSDPool(chainId: ChainId, poolProvider: IV2PoolProvider): Promise<Pair> {
    ...
    const usdPools = _.map<Token, [Token, Token]>(usdTokens, (usdToken) => [usdToken, WRAPPED_NATIVE_CURRENCY[chainId]])
